...
let myName: string = "Muhammad Ishaq"

const message: string = `Hello ${myName}, would you like to learn some Javascript today?`;
console.log(message);
...
const famous_person_message: string = `In the middle of every difficulty lies opportunity. - ${famous_person}`;
...
const friendNames: string[] = ['ishaq', 'hasnat', 'abbas'];
...
friendNames.forEach((friendName: string) => {
  console.log(`I would like to have a conversation with Mr. ${friendName}.`);
});
...
const vehicles: string[] = ['car', 'motorcycle', 'bicycle'];
...
const guests: string[] = ['ishaq', 'hasnat', 'abbas'];
...
const unableToAttend: string | undefined = guests.pop();
guests.push('suleman');
guests.forEach((guest: string) => {
  console.log(`Dear ${guest}, you are invited to dinner.`);
});
...
const dinnerGuests: string[] = ['Albert Einstein', 'Leonardo da Vinci', 'Marie Curie'];
...
const mountains: string[] = ['Mount Everest', 'K2', 'Kangchenjunga'];
const rivers: string[] = ['Amazon', 'Nile', 'Yangtze'];
const countries: string[] = ['Pakistan', 'Brazil', 'China'];
const cities: string[] = ['Paris', 'New York', 'Tokyo'];
const languages: string[] = ['English', 'Spanish', 'Mandarin'];
...
const car: { manufacturer: string; model: string; year: number; color: string } = {
    manufacturer: 'Toyota',
    model: 'Camry',
    year: 2022,
    color: 'Silver',
  ...
  const movie: { title: string; director: string; duration: number } = {
...
const new_users: string[] = ["user5", "user6", "admin", "user7", "user8"];

new_users.forEach((new_user: string) => {
  if (current_users.map((user: string) => user.toLowerCase()).includes(new_user.toLowerCase())) {
    console.log(`Sorry, ${new_user} will need to enter a new username.`);
  } else {
...
const favorite_pizzas: string[] = ["pepperoni", "cheese", "vegetable"];
...
type Album = {
  ...
  const album1: Album = make_album("Billie Eilish", "When We All Fall Asleep, Where Do We Go?", 14);
  const album2: Album = make_album("Taylor Swift", "Evermore");
  const album3: Album = make_album("Ed Sheeran", "÷ (Divide)", 16);
  console.log(album1);
  console.log(album2);
  console.log(album3);
...
type Car = {
  ...
  function make_car(manufacturer: string, model: string, ...options: string[]): Car {
    const car: Car = {
  ...
  const car1: Car = make_car("Toyota", "Camry", "sunroof", "leather seats");
  const car2: Car = make_car("Honda", "Civic");
  console.log(car1);
  console.log(car2);
